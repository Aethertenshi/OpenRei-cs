...
/// size-quantizing protection, LRU eviction, string texture caching, and text measurement using SDL3_ttf.
/// </summary>
public unsafe class Font : IDisposable
{
    ...
    {
        if (string.IsNullOrEmpty(text) || color.A == 0) return null;

        string cacheKey = $"{text}_{fontSize:F1}_{color.R}_{color.G}_{color.B}_{color.A}";
        if (_stringTextureCache.TryGetValue(cacheKey, out var cachedTex))
        {
            return cachedTex;
        }

        TTF_Font* fontHandle = GetHandle(fontSize);
        if (fontHandle == null) return null;

        SDL_Color sdlColor = new SDL_Color { r = color.R, g = color.G, b = color.B, a = color.A };
        ...
        int width = rgbaSurf->w;
        int height = rgbaSurf->h;
        byte[] pixels = new byte[width * height * 4];
        fixed (byte* pDst = pixels)
        {
            Buffer.MemoryCopy((void*)rgbaSurf->pixels, pDst, pixels.Length, pixels.Length);
        }
